...
/** Copied from http://www-nlpir.nist.gov/projects/irf/irf.html
 *
 * A buffered version of RandomAccessFile.
 *<br><br> 
 * Instances of this class realise substantial speed increases over 
 * RandomAccessFile through the use of buffering. This is a subclass of 
 * Object, as it was not possible to subclass RandomAccessFile because 
 * many of the methods are final. However, if it is necessary to use 
 * BufferedRandomAccessFile and RandomAccessFile interchangeably, both 
 * classes implement the DataInput and DataOutput interfaces. 
 * <br><br>
 * By Alex McManus, based on Sun's source code for RandomAccessFile.
 * Copyright 1997 Centre For Intelligent Systems,
 * Univeristy of Wales, Aberystwyth.
 * <br><br>
 * Modified by National Institute of Standards and Technology for use
 * in the Information Retrieval Framework (IRF):
 * <ul>
 * <li> 99/10/01 line 223: endOfSize unset if staying in the buffer
 * during a seek().
 * <li> 99/11/03 line 225: seeking the end of the file doesn't flush
 * the buffer anymore.
 * <li> 99/11/03 line 589: when writing data longer than the buffer,
 * this one was becoming unconsistent. It is now fixed: the buffer is
 * empty after such a writing but its associated values allow it to be
 * used.
 * <li> 99/11/16  Removed method public final void 
 * writeBytes(FastStringBuffer s) to eliminate dependency on 
...
public class BufferedRandomAccessFile extends Object implements DataInput, DataOutput {
    ...
    *                               security exception. 
    */ 
   public BufferedRandomAccessFile( String filename, int mode ) 
   throws IOException { 
    ...
    *                               security exception. 
    */ 
   public BufferedRandomAccessFile( String filename, int mode, int bufferSize ) 
   throws IOException { 
      this.mode = mode; 
   ...
   throws IOException { 
      ...
      if( pos >= bufferStart && pos < dataEnd ) { 
         filePosition = pos;
	 endOfFile = false; /* Fix by G. Sanseau. I think that if we ARE
			       at the end of the file, pos == dataEnd. */
	 return; 
      } else if (dataSize < buffer.length &&
		 pos == dataEnd &&
		 dataEnd == getLength()) {
	/* Improvement by G. Sanseau: Seeking the end of the file
	   to add things in it without flushing the buffer if there
	   is no need for it. */
	filePosition = pos;
	endOfFile = true;
	return;
   ...
   throws IOException { 
      ...
      // ...or write a lot of data... 
	   ...
	   flush( ); 
         ...
         filePosition += len;
	 /* Fix by G. Sanseau: The following code was in if(bufferModified),
	    with the first two variables set to 0. Next write wouldn't work
	    then, because the offset in the buffer for write is
	    (filePosition - bufferStart). */
	 bufferStart = dataEnd = filePosition;
	 dataSize = 0; 
      } 
   } 
   ...
   public final void writeUTF(String str) throws IOException { 
      ...
      if (utflen > 65535) 
         throw new UTFDataFormatException("BRAF.writeUTF: string to be written too long.");		   
   ...
   /** 
    * Test the byte operations of the BufferedRandomAccessFile class. These are 
   ...
   public static void testBytes( String filename, int bufferSize ) { 
      ...
      try { 
         ...
         // Create a test file. 
         BufferedRandomAccessFile outFile = new BufferedRandomAccessFile( filename,  
            BufferedRandomAccessFile.WRITE | 
            BufferedRandomAccessFile.CREATE, bufferSize ); 
         try { 
         ...
         BufferedRandomAccessFile inoutFile = new BufferedRandomAccessFile( filename,  
            BufferedRandomAccessFile.READ | 
            BufferedRandomAccessFile.WRITE, bufferSize ); 
         ...
         BufferedRandomAccessFile inFile = new BufferedRandomAccessFile( filename,  
            BufferedRandomAccessFile.READ, bufferSize ); 
   ...
   /** 
    * Test the block operations of the BufferedRandomAccessFile class. These 
   ...
   public static void testBlocks( String filename ) { 
      ...
      try { 
         ...
         BufferedRandomAccessFile outFile = new BufferedRandomAccessFile( 
                               filename, BufferedRandomAccessFile.WRITE | 
                               BufferedRandomAccessFile.CREATE, bufferSize ); 
         ...
         BufferedRandomAccessFile inFile = new BufferedRandomAccessFile( 
                        filename, BufferedRandomAccessFile.READ, bufferSize ); 
   ...
   /** 
    * Benchmark the performance of the new BufferedRandomAccessFile class. Its 
    * speed is compared to that of a RandomAccessFile, based on reading and 
   ...
   public static void benchmark( String filename, int bufferSize ) { 
      ...
      try { 
         BufferedRandomAccessFile inFile = new BufferedRandomAccessFile( filename,  
            BufferedRandomAccessFile.READ, bufferSize ); 
         BufferedRandomAccessFile outFile = new BufferedRandomAccessFile( "temp.data",  
            BufferedRandomAccessFile.WRITE | 
            BufferedRandomAccessFile.CREATE, bufferSize ); 
         ...
         System.out.println( ". BufferedRandomAccessFile elapsed time=" +  
  ...
  /** 
   * Test the BufferedRandomAccessFile class. This involves testing the byte 
  ...
  public static void main( String argv[] ) { 
      ...
      if( argv.length < 1 ) { 
         System.err.println( "Usage: BufferedRandomAccessFile <filename> [buffer.length] [benchmark | test]" ); 
         System.exit( -1 ); 
      ...
      if( test ) { 
         BufferedRandomAccessFile.testBytes( "temp.data", bufferSize ); 
         BufferedRandomAccessFile.testBlocks( "temp.data" ); 
      } 
      if( benchmark ) { 
         BufferedRandomAccessFile.benchmark( filename, bufferSize ); 
      } 
